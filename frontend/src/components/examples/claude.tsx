...
const ComposerPrimaryAction: FC = () => {
  return (
    <>
      ...
      <AuiIf
        condition={(state) =>
          !state.thread.isRunning && state.composer.dictation != null
        }
      >
      ...
      <AuiIf
        condition={(state) =>
          ...
          state.composer.dictation == null &&
          !state.composer.isEmpty
        }
      >
...
const useAttachmentSrc = () => {
  const { file, src } = useAuiState(
    useShallow((state): { file?: File; src?: string } => {
      ...
      const src = state.attachment.content?.filter(
...
const ClaudeAttachment: FC = () => {
  ...
  return (
    <AttachmentPrimitive.Root className="group/thumbnail relative">
      ...
      >
        {isImage && src ? (
          // Blob URLs are local attachment previews and cannot use next/image.
